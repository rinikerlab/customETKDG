...
def get_noe_restraint_bmat(mol, df):
    ...
    for key, val in noe_atom_pair2upper_distance.items():
        ...
        for item in product(a,b):
            if bmat[max(item), min(item)] > tolerance*val: #updates the lower bound only if it is smaller than NOE suggests
                # print(bmat[max(item), min(item)] , val)
                bmat[max(item), min(item)] = tolerance*val
            # print(item, bmat[min(item), max(item)], val)
            bmat[min(item), max(item)] = tolerance*val # upper bound?
