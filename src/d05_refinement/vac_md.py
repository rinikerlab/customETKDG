...
reporter_path = os.getcwd()+'/'+str(simtime_ns)+'ns_vac_reporters/'
...
out_mol = minimise_energy_confs(rdmol, noe_restraints=rdmol_df, allow_undefined_stereo=True, restraints=True,
                                use_mlddec=True, implicitSolvent=False, solventDielectric=solventDielectric,
...
Chem.MolToPDBFile(out_mol, f"41_{simtime_ns}ns_vac_{min(confs_index)}-{max(confs_index)}idx_{pdb_path}")
